...
public class SetStolenC2SPacket {
    private final CursedTechnique technique;

    public SetStolenC2SPacket(CursedTechnique technique) {
        this.technique = technique;
    }

    public SetStolenC2SPacket(FriendlyByteBuf buf) {
    ...
    public void handle(Supplier<NetworkEvent.Context> supplier) {
        ...
        ctx.enqueueWork(() -> {
            ...
            if (cap.getStolen().contains(this.technique)) {
                cap.setCurrentStolen(this.technique);
            }
        });
