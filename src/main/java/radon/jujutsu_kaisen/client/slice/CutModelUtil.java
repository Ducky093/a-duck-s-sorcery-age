    ...
    private static RigidBody.VertexData[] cutAndCapConvex(RigidBody.Triangle[] triangles, float[] plane) {
        ...
        if (!clippedEdges.isEmpty()) {
            ...
            while (!clippedEdges.isEmpty()) {
                Optional<Vec3> next = getNext(clippedEdges, orderedClipVertices.get(orderedClipVertices.size() - 1));
            ...
            for (int i = 0; i < cap.length; i++) {
                ...
                cap[i] = new RigidBody.Triangle(orderedClipVertices.get(0), orderedClipVertices.get(i + 2), orderedClipVertices.get(i + 1),
                        new float[] { uv1.x, uv1.y, uv2.x, uv2.y, uv3.x, uv3.y },
                        new int[] { 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF });
                side1.add(new RigidBody.Triangle(orderedClipVertices.get(0), orderedClipVertices.get(i + 2), orderedClipVertices.get(i + 1), new float[6], new int[3]));
                side2.add(new RigidBody.Triangle(orderedClipVertices.get(0), orderedClipVertices.get(i + 1), orderedClipVertices.get(i + 2), new float[6], new int[3]));
            }
