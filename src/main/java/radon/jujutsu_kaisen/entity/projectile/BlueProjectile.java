...
public class BlueProjectile extends JujutsuProjectile {
    ...
    private void pullEntities() {
        ...
        if (this.getOwner() instanceof LivingEntity owner) {
            for (Entity entity : this.level().getEntities(owner, bounds)) {
                ...
                if (entity instanceof MahoragaEntity maho ) {
                    ITenShadowsData cap = maho.getCapability(TenShadowsDataHandler.INSTANCE).resolve().orElseThrow();
                    if (cap.isAdaptedTo(JJKAbilities.BLUE_STILL.get())  ) {
                        return;
                    }
                }
    ...
    private void spawnParticles() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            this.level().addParticle(new TravelParticle.TravelParticleOptions(center.toVector3f(), ParticleColors.DARK_BLUE, radius * 0.075F, 1.0F, true, true, 5),
        ...
        for (int i = 0; i < count; i++) {
            ...
            this.level().addParticle(new TravelParticle.TravelParticleOptions(center.toVector3f(), ParticleColors.LIGHT_BLUE, radius * 0.05F, 1.0F, true, true, 5),
                    x, y, z, 0.0D, 0.0D, 0.0D);
        }
    }
