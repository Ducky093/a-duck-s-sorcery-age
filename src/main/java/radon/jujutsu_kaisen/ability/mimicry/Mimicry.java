...
public class Mimicry extends Ability implements Ability.IToggled, Ability.IAttack {
    ...
    public boolean attack(DamageSource source, LivingEntity owner, LivingEntity target) {
        ...
        if (copied == null || current == null || ownerCap.hasTechnique(copied) || (copied == CursedTechnique.BRAIN_TRANSPLANT && !ConfigHolder.SERVER.mimicryBodyStealCompat.get() ) || copied == CursedTechnique.MYTHICAL_BEAST_AMBER ) return false;
        if (owner instanceof Player && ConfigHolder.SERVER.playerMimicry.get() && !(target instanceof Player) ) return false;
        if (current != copied) {
